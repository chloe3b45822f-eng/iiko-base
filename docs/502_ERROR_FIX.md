# Решение проблемы 502 Bad Gateway

## 📌 Описание проблемы

При попытке открыть сайт vezuroll.ru в браузере появляется ошибка:
```
502 Bad Gateway
nginx/1.24.0 (Ubuntu)
```

## 🔍 Причины ошибки 502

Ошибка 502 Bad Gateway означает, что Nginx не может подключиться к upstream-сервису (PHP-FPM или Python backend). Основные причины:

1. **PHP-FPM не запущен** - самая частая причина
2. **Python backend не запущен**
3. **Неправильный путь к PHP-FPM сокету**
4. **Недостаточно памяти на сервере**
5. **Ошибки в конфигурации Nginx**

## ✅ Решение

### Шаг 1: Проверка статуса всех сервисов

```bash
# Проверка PHP-FPM
sudo systemctl status php8.1-fpm
# или для других версий PHP
sudo systemctl status php-fpm

# Проверка Python backend
sudo systemctl status iiko-backend

# Проверка Nginx
sudo systemctl status nginx
```

### Шаг 2: Запуск остановленных сервисов

#### Если PHP-FPM не запущен:
```bash
# Для PHP 8.1
sudo systemctl start php8.1-fpm
sudo systemctl enable php8.1-fpm

# Для других версий
sudo systemctl start php-fpm
sudo systemctl enable php-fpm
```

#### Если Python backend не запущен:
```bash
sudo systemctl start iiko-backend
sudo systemctl enable iiko-backend
```

#### Если Nginx не запущен:
```bash
sudo systemctl start nginx
sudo systemctl enable nginx
```

### Шаг 3: Проверка логов на ошибки

```bash
# Логи Nginx
sudo tail -50 /var/log/nginx/error.log
sudo tail -50 /var/log/nginx/iiko-base-error.log

# Логи PHP-FPM
sudo tail -50 /var/log/php8.1-fpm.log
# или
sudo journalctl -u php8.1-fpm -n 50

# Логи Python backend
sudo journalctl -u iiko-backend -n 50
```

### Шаг 4: Проверка конфигурации Nginx

```bash
# Проверка синтаксиса
sudo nginx -t

# Если есть ошибки, исправьте их в конфигурации
sudo nano /etc/nginx/sites-available/iiko-base
```

### Шаг 5: Проверка PHP-FPM сокета

```bash
# Проверка существования сокета
ls -la /var/run/php/php8.1-fpm.sock

# Если сокет не существует, проверьте конфигурацию PHP-FPM
sudo nano /etc/php/8.1/fpm/pool.d/www.conf

# Найдите строку с listen и убедитесь что она указывает на сокет
# listen = /var/run/php/php8.1-fpm.sock
```

### Шаг 6: Перезапуск всех сервисов

После исправления проблем перезапустите все сервисы:

```bash
# Автоматическая перезагрузка всех сервисов
cd /var/www/iiko-base
sudo ./scripts/deploy.sh
```

Или вручную:

```bash
sudo systemctl restart php8.1-fpm
sudo systemctl restart iiko-backend
sudo systemctl restart nginx
```

## 🚀 Быстрое решение (в большинстве случаев)

В 90% случаев проблема решается простым запуском PHP-FPM:

```bash
sudo systemctl start php8.1-fpm
sudo systemctl enable php8.1-fpm
sudo systemctl restart nginx
```

## 📊 Диагностика

### Проверка доступности сервисов

```bash
# Проверка PHP-FPM через сокет
sudo -u www-data SCRIPT_FILENAME=/var/www/iiko-base/frontend/public/index.php \
     REQUEST_METHOD=GET \
     cgi-fcgi -bind -connect /var/run/php/php8.1-fpm.sock

# Проверка Python backend
curl http://localhost:8000/health

# Проверка Nginx
curl -I http://localhost/
```

### Проверка ресурсов сервера

```bash
# Проверка использования памяти
free -h

# Проверка использования диска
df -h

# Проверка процессов
ps aux | grep -E 'php-fpm|uvicorn|nginx'
```

## 🔧 Дополнительные исправления

### Если используется другая версия PHP

Если на вашем сервере установлена другая версия PHP (например, 7.4 или 8.0), обновите конфигурацию Nginx:

```bash
# Узнайте версию PHP
php -v

# Найдите путь к PHP-FPM сокету
sudo find /var/run/php/ -name "*.sock"

# Обновите конфигурацию Nginx
sudo nano /etc/nginx/sites-available/iiko-base

# Измените строку fastcgi_pass на актуальный путь
# fastcgi_pass unix:/var/run/php/php8.0-fpm.sock;
```

### Увеличение таймаутов

Если сервис медленный, увеличьте таймауты в конфигурации Nginx:

```nginx
location ~ \.php$ {
    # ... другие настройки ...
    
    # Увеличенные таймауты
    fastcgi_connect_timeout 60s;
    fastcgi_send_timeout 60s;
    fastcgi_read_timeout 60s;
}
```

## 📝 Проверка после исправления

После применения исправлений проверьте:

1. **Веб-интерфейс**: откройте http://vezuroll.ru в браузере
2. **API**: откройте http://api.vezuroll.ru/health
3. **Документация API**: откройте http://api.vezuroll.ru/api/v1/docs

## ⚠️ Частые ошибки

### Ошибка: "connect() to unix:/var/run/php/php8.1-fpm.sock failed"

**Причина**: PHP-FPM не запущен или сокет не существует

**Решение**:
```bash
sudo systemctl start php8.1-fpm
sudo systemctl enable php8.1-fpm
```

### Ошибка: "upstream timed out"

**Причина**: PHP-FPM или backend работает слишком медленно

**Решение**: Увеличьте таймауты в конфигурации Nginx (см. выше)

### Ошибка: "Permission denied"

**Причина**: Nginx не имеет прав доступа к PHP-FPM сокету

**Решение**:
```bash
# Проверьте права доступа к сокету
ls -la /var/run/php/php8.1-fpm.sock

# Убедитесь, что Nginx запущен от пользователя www-data
ps aux | grep nginx

# Проверьте настройки в /etc/php/8.1/fpm/pool.d/www.conf
# user = www-data
# group = www-data
# listen.owner = www-data
# listen.group = www-data
```

## 📞 Поддержка

Если проблема не решена после выполнения всех шагов:

1. Соберите логи всех сервисов
2. Сделайте скриншот ошибки в браузере
3. Создайте issue в репозитории GitHub с подробным описанием

## 🔗 Дополнительные ресурсы

- [DATABASE_ERRORS.md](./DATABASE_ERRORS.md) - решение проблем с PostgreSQL
- [FAQ.md](./FAQ.md) - часто задаваемые вопросы
- [INSTALLATION.md](./INSTALLATION.md) - руководство по установке
