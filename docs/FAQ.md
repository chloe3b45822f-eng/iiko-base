...
- По умолчанию пользователь не создан: сначала зарегистрируйте его через API:
  ```bash
  # создаст пользователя с паролем secret123
  ...
  ```
- После этого авторизуйтесь на `/login` под логином `admin` и паролем `secret123` и вы будете перенаправлены на `/admin`.
