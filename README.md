...
🔴 **КРИТИЧЕСКИ ВАЖНО:** Пароль по умолчанию известен публично. **ОБЯЗАТЕЛЬНО** смените его сразу после первого входа в продакшн системе! Для продакшн рекомендуется установить безопасный пароль через переменную окружения `DEFAULT_ADMIN_PASSWORD` перед первым запуском.
